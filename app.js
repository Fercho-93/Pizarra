(function () {
  ...
  function updateSuggestions(input) {
    ...
    container.innerHTML = matches.map(player => `<button class="suggestion" data-select-player="${player.id}" data-context="${context}"><span><b>${escapeHtml(player.name)}</b><small>${escapeHtml(playerMeta(player))}</small></span><span>›</span></button>`).join("");
  ...
  render();
  if ("serviceWorker" in navigator && location.protocol !== "file:") navigator.serviceWorker.register("./service-worker.js?v=3").catch(() => {});
})();
